...
int CLtext::parseTxt(ifstream &textDatei){
    ...
    //Hier habe ich vorab getestet, ob die einzelnen Datensätze in den Objekten gespeichert werden
...
void CLtext::druckeTxt() //drucke Funktion für den Text Parser
{
    for(int i=0;i<getAnzahlProdukt();i++)
        cout << "ID: " << getKleidungsID(i) << " Groesse: " << getKleidungsGroesse(i) << " Bestand: " << getKleidungsBestand(i) << endl;
}
